...
export class Chart extends React.Component {
  ...
  connect = () => {
    this.socket = socketIOClient(
      `?sensor=${this.props.sensorId}&time=${this.props.time}`
    );
    this.socket.on('DbData',(data)=>{
      console.log(data)
      for(let i in data){
        console.log(data[i])
        this.storeReading(JSON.stringify({
          timestamp: new Date(data[i].mongo_time).getTime(),
          value: data[i][this.props.listen],
          zscore: 0
        }))
      }
    })
    this.socket.on('reading'+this.props.listen, (data)=>{
      console.log(this.props.listen);
      this.storeReading(data);
    });
